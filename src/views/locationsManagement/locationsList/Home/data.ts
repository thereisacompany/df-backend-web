...
//api
import { updateArticle } from '/@/api/page/locations/locationsList';
...
export const getColumns = (categorys, handleLoading, categorysregions) => {
  const columns: BasicColumn[] = [
    ...
    // },
    {
      title: t('component.locationsManagement.title'),
      dataIndex: 'body',
      width: 80,
      customRender: ({ record }) => {
        let content = '';
    ...
    {
      title: t('component.locationsManagement.type'),
      dataIndex: 'category',
      width: 80,
      customRender: ({ record }) => {
        let text = '';
        const values = XEUtils.filter(categorys, (item) => item.value === record.type);
    ...
    {
      title: t('component.locationsManagement.regions'),
      dataIndex: 'categorysregions',
      width: 80,
      customRender: ({ record }) => {
        let text = '';
        const values = XEUtils.filter(categorysregions, (item) => item.value === record.regions);
        if (values.length > 0) text = values[0].label;
    ...
    {
      title: t('component.locationsManagement.address'),
      dataIndex: 'address',
      width: 80,
      // customRender: ({ record }) => {
      //   let text = '';
      //   const values = record.address;
      //   if (values.length > 0) text = values[0].label;
